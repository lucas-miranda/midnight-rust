...
pub enum RenderBackendBuildError {
    InstanceFailed(wgpu_hal::InstanceError),
    SurfaceFailed(wgpu_hal::InstanceError),
    AdapterNotFound,
    PresentationNotSupported,
    LogicalDeviceOpenFailed(wgpu_hal::DeviceError),
    SurfaceConfigureFailed(wgpu_hal::SurfaceError),
    PipelineLayoutFailed(wgpu_hal::DeviceError),
    PipelineFailed(wgpu_hal::PipelineError),
    CommandEncoderFailed(wgpu_hal::DeviceError),
}
